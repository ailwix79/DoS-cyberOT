...
from scapy.fields import LEShortField, ThreeBytesField, ByteField, ShortField, BitField, X3BytesField
from cotp import COTP_Layer, COTP_TCP_Data, COTP_LLC_Data
...
class S7COMM(Packet):
    name = "S7COMM"
    fields_desc = [
        ByteField("protocol_id", 0x32), # 1 byte
        ByteField("rosctr", 1), # 1 byte       # Remote operating service control
        ShortField("reserved", 0x0000), # 2 bytes
        LEShortField("pdu_ref", 0x0000), # 2 bytes
        ShortField("param_length", 0x0000), # 2 bytes
        ShortField("data_length", 0x0000) # 2 bytes
        ]

class S7COMM_Item(Packet):
    name = "S7COMM_Item"
    fields_desc = [
        ByteField("varspec", 0x12), # 1 byte
        ByteField("varspec_length", 10), # 1 byte       # Remote operating service control
        ByteField("syntaxid", 0x10), # 1 bytes
        ByteField("transp_size", 1), # 1 bytes
        ShortField("length", 1), # 2 bytes
        ShortField("db", 0), # 2 bytes
        ByteField("area", 0x83), # 1 bytes
        X3BytesField("addressBit", 0x0003e0) # 3 bytes
        ]

class S7COMM_Data_Item(Packet):
    name = "S7COMM_Data_Item"
    fields_desc = [
        ByteField("returncode", 0x00), # 1 byte
        ByteField("transportsize", 0x03), # 1 byte       # Remote operating service control
        ShortField("length", 1), # 2 bytes
        ByteField("data", 1) # 1 bytes
        ]
...
class S7COMM_Job_Item(Packet):
    name = "S7COMM_Job_Func"
    fields_desc = [
        ByteField("function", 0),
        ByteField("item_count", 1)
    ]
...
class S7COMM_Job_Connect(Packet):
    ...
    fields_desc = [
        ...
        ShortField("max_amq_calling", 0x0001),
        ShortField("max_amq_called", 0x0003),
        ShortField("pdu_length", 0x00f0)
    ]
